...
export function AssetsContent() {
  ...
  if (error || !data) {
    return (
      <div className="rounded-xl border border-red-100 bg-red-50 p-4 text-base text-red-600">
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        <div className="rounded-2xl border border-amber-200 bg-gradient-to-b from-amber-50 to-white p-6">
          ...
          <div className="mb-6 flex items-start gap-0">
            {[
              ...
              <div key={s.title} className="flex flex-1 flex-col items-center gap-1.5">
                ...
                <p className="text-center text-sm text-neutral-500">{s.desc}</p>
              </div>
            ))}
          </div>
          ...
          <Link
            href="/assets/upload"
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-amber-500 px-4 py-3 text-base font-semibold text-white shadow hover:bg-amber-600 active:scale-95 transition-transform"
          >
    ...
    <div className="flex flex-col gap-6">
      <header className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 flex-col gap-2">
          <Link
            href="/assets/history"
            className="flex items-center gap-1.5 whitespace-nowrap rounded-xl border border-neutral-200 bg-white px-3.5 py-2 text-base font-medium text-neutral-700 shadow-sm hover:bg-neutral-50 active:scale-95 transition-transform"
          >
          ...
          <Link
            href="/assets/upload"
            className="flex items-center gap-1.5 whitespace-nowrap rounded-xl bg-amber-500 px-3.5 py-2 text-base font-semibold text-white shadow-sm hover:bg-amber-600 active:scale-95 transition-transform"
          >
            <svg className="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
      ...
      {totalLiveKrw > 0 && (
        <section className="rounded-xl border border-neutral-200 bg-neutral-50 p-4">
          <p className="mb-2.5 text-sm font-medium uppercase tracking-wide text-neutral-500">
            내 자산으로 시뮬레이터 실행
          </p>
          <div className="flex flex-wrap gap-2">
            <Link href={`/sim/fire?currentAssets=${totalLiveKrw}`} className="flex items-center gap-1.5 rounded-lg border border-neutral-200 bg-white px-3 py-2 text-base font-medium text-neutral-700 shadow-sm hover:bg-neutral-50 active:scale-95 transition-transform">
              🎯 FIRE 계산기
            </Link>
            <Link href={`/sim/depletion?startAssets=${totalLiveKrw}`} className="flex items-center gap-1.5 rounded-lg border border-neutral-200 bg-white px-3 py-2 text-base font-medium text-neutral-700 shadow-sm hover:bg-neutral-50 active:scale-95 transition-transform">
              📉 자산 고갈 시뮬
            </Link>
            {yearlyDivKrw > 0 && (
              <Link href={`/sim/retire-cashflow?dividendYearly=${yearlyDivKrw}`} className="flex items-center gap-1.5 rounded-lg border border-neutral-200 bg-white px-3 py-2 text-base font-medium text-neutral-700 shadow-sm hover:bg-neutral-50 active:scale-95 transition-transform">
                💰 은퇴 현금흐름
              </Link>
            )}
          </div>
