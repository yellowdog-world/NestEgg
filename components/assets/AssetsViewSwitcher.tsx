...
function GainBadge({ evalKrw, costKrw }: { evalKrw: number; costKrw: number }) {
  if (costKrw <= 0 || evalKrw <= 0) return <span className="text-base text-neutral-300">—</span>;
  const gain = evalKrw - costKrw;
  const pct = (gain / costKrw) * 100;
  const pos = gain >= 0;
  return (
    <span className={`text-base font-semibold tabular-nums ${pos ? "text-red-500" : "text-blue-500"}`}>
      {pos ? "+" : ""}{pct.toFixed(1)}%
    </span>
  );
}
    ...
    <section>
      ...
      <div className="mb-4 flex gap-1 rounded-lg bg-neutral-100 p-1">
        {TABS.map((t) => (
          <button
            ...
            className={`flex-1 whitespace-nowrap rounded-md px-3 py-1.5 text-base font-medium transition-colors ${
              tab === t.id
            ...
            <div key={type} className="rounded-xl border border-neutral-200 bg-white p-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className={`inline-flex rounded-full px-2.5 py-0.5 text-sm font-medium ${TYPE_COLOR[type] ?? "bg-neutral-100 text-neutral-600"}`}>
                    {ACCOUNT_LABEL[type] ?? type}
                  </span>
                  <span className="text-sm text-neutral-400">{count}개 계좌</span>
                </div>
                <GainBadge evalKrw={evalKrw} costKrw={costKrw} />
              </div>
              <div className="mt-3 grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm text-neutral-400">투자원금</p>
                  <p className="mt-0.5 text-base font-semibold tabular-nums text-neutral-700">{fmtKRWShort(costKrw)}</p>
                </div>
                <div>
                  <p className="text-sm text-neutral-400">평가금</p>
                  <p className="mt-0.5 text-base font-semibold tabular-nums text-neutral-900">{fmtKRWShort(evalKrw)}</p>
                </div>
              </div>
              <PctBar pct={pct} />
            </div>
              ...
              <div key={broker} className="rounded-xl border border-neutral-200 bg-white overflow-hidden">
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-base font-semibold text-neutral-900">{broker}</span>
                        <span className="text-sm text-neutral-400">{count}개 계좌</span>
                        <div className="flex flex-wrap gap-1">
                          {types.map((t) => (
                            <span key={t} className="rounded bg-neutral-100 px-1.5 py-0.5 text-sm text-neutral-500">{t}</span>
                          ))}
                        </div>
                      </div>
                    </div>
                        ...
                        <div key={account.id} className="flex items-center justify-between gap-3 px-4 py-3 hover:bg-neutral-50">
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className={`inline-flex rounded-full px-2 py-0.5 text-sm font-medium ${TYPE_COLOR[account.type] ?? "bg-neutral-100 text-neutral-600"}`}>
                                {ACCOUNT_LABEL[account.type] ?? account.type}
                              </span>
                              {account.nickname && (
                                <span className="text-sm text-neutral-500">{account.nickname}</span>
                              )}
                            </div>
                            {capturedAt && (
                          ...
                          <div className="text-right shrink-0">
                            ...
                            {retPct != null && (
                              <p className={`text-sm font-medium tabular-nums ${pos ? "text-red-500" : "text-blue-500"}`}>
                          ...
                          <Link
                            ...
                            className="shrink-0 rounded-md border border-neutral-200 px-2.5 py-1.5 text-sm text-neutral-500 hover:bg-neutral-50"
      ...
      {tab === "security" && (
        <div className="flex flex-col gap-2.5">
              ...
              <>
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {isKrx && sec.name && (
                      <p className="text-base text-neutral-500 truncate">{sec.name}</p>
                    )}
                  </div>
                ...
                <div className="mt-1.5 flex items-center justify-between gap-2">
                  ...
                  {pct != null ? (
                    <span className={`shrink-0 text-base font-medium tabular-nums ${pos ? "text-red-500" : "text-blue-500"}`}>
                ...
                {share > 0.1 && (
                  <div className="mt-2.5 flex items-center gap-2">
                    ...
                    <span className="text-sm text-neutral-400 tabular-nums w-8 text-right">{share.toFixed(1)}%</span>
                  </div>
                )}
              </>
            );
          ...
          {bySecurity.length === 0 && (
            <p className="rounded-xl border border-dashed border-neutral-200 py-10 text-center text-base text-neutral-400">
              종목 데이터가 없습니다.
            </p>
          )}
        </div>
...
function EmptyState() {
  return (
    <div className="col-span-full rounded-xl border border-dashed border-neutral-300 bg-white p-8 text-center text-base text-neutral-500">
      아직 계좌가 없어요.{" "}
