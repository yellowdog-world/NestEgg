...
export function SimEmbed({ simulator }: { simulator: string; preset?: Record<string, unknown> }) {
  ...
  if (!meta) {
    return (
      <div className="my-4 rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-800">
  ...
  return (
    ...
    >
      ...
      <span className="text-sm text-neutral-700">{meta.description}</span>
      <span className="text-xs text-amber-700">→ /sim/{meta.path}</span>
    </Link>
  );
}
