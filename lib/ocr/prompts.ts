...
검증 4: avg_price 크기 검증 — 평단가 vs 평가손익 혼동 방지
  ★★★ 가장 흔한 오류: 평가손익(총액)을 avg_price(주당 단가)에 넣는 실수 ★★★
  - avg_price는 반드시 "1주당 가격"이어야 함. 총액(수량 × 단가)이 아님.
  - avg_price > market_price × 5 이면 → avg_price에 평가손익 총액이 들어간 것.
    올바른 계산: avg_price = market_price − (잘못들어온값 ÷ quantity)
  - 예) market=292.22, qty=30, 잘못된avg=4160.11
    → 올바른avg = 292.22 − (4160.11 ÷ 30) = 292.22 − 138.67 = 153.55
  - avg_price < 0 이면 → profit_loss가 avg_price에 잘못 들어간 것. avg_price = null로 처리.

  ★ 반드시: avg_price는 market_price와 비슷한 자릿수여야 함 (±10배 이내)
